...
// Shared with the interpreter and VUops side-effect guards during MVU_DIFF.
...
// When set, recMacMVU1::Execute runs the *interpreter* as the real (known-good)
// VU1 and microVU1 as a side-effect-suppressed shadow, comparing VF/VI/ACC/Mem
// and logging the first diverging program's start PC. g_mvuShadowRun gates the
...
#include "arm64/mac/aVU_Analyze.inl"
...
#include "arm64/mac/aVU_Alloc.inl"
...
#include "arm64/mac/aVU_Clamp.inl"
...
#include "arm64/mac/aVU_Misc.inl"
...
#include "arm64/mac/aVU_Upper.inl"
...
#include "arm64/mac/aVU_Lower.inl"
...
#include "arm64/mac/aVU_Tables.inl"
...
#include "arm64/mac/aVU_Flags.inl"
...
#include "arm64/mac/aVU_Branch.inl"
...
//------------------------------------------------------------------
// recMacMVU0 / recMacMVU1
//------------------------------------------------------------------

recMacMVU0 CpuMicroVU0;
recMacMVU1 CpuMicroVU1;

recMacMVU0::recMacMVU0() { m_Idx = 0; IsInterpreter = false; }
recMacMVU1::recMacMVU1() { m_Idx = 1; IsInterpreter = false; }

void recMacMVU0::Reserve()
{
	mVUinit(microVU0, 0);
}
void recMacMVU1::Reserve()
{
	mVUinit(microVU1, 1);
	vu1Thread.Open();
}

void recMacMVU0::Shutdown()
{
	mVUclose(microVU0);
}
void recMacMVU1::Shutdown()
{
	if (vu1Thread.IsOpen())
		vu1Thread.WaitVU();
	mVUclose(microVU1);
...
void recMacMVU0::Reset()
{
	mVUreset(microVU0, true);
}

void recMacMVU0::Step()
{
}

void recMacMVU1::Reset()
{
	vu1Thread.WaitVU();
...
void recMacMVU0::SetStartPC(u32 startPC)
{
	VU0.start_pc = startPC;
}

void recMacMVU0::Execute(u32 cycles)
{
...
void recMacMVU1::SetStartPC(u32 startPC)
{
	VU1.start_pc = startPC;
}

void recMacMVU1::Step()
{
}
...
void recMacMVU1::Execute(u32 cycles)
{
	if (!THREAD_VU1)
	{
		if (!(VU0.VI[REG_VPU_STAT].UL & 0x100))
...
void recMacMVU0::Clear(u32 addr, u32 size)
{
	mVUclear(microVU0, addr, size);
}
void recMacMVU1::Clear(u32 addr, u32 size)
{
	mVUclear(microVU1, addr, size);
}

void recMacMVU1::ResumeXGkick()
{
	if (!(VU0.VI[REG_VPU_STAT].UL & 0x100))
		return;
...
#include "arm64/mac/aVU_Compile.inl"
